...
from CrossVLT.bert_crossvlt import *
from CrossVLT.CrossVLT import *
...
class Text(nn.Module):
    ...
    def forward(self, x):
        tokenizer = BertTokenizer.from_pretrained(r'/home/ltran/LOTVS-CAP/bert/modeling_bert/bert-base-uncased-vocab.txt')
        max_len = 15
        res=[]
        for text in x:
...
class improved_accident(nn.Module):
    def __init__(self,h_dim,n_layers,depth,adim,heads,num_tokens,c_dim,s_dim1,s_dim2,keral,num_class):
        ...
        self.cross_vlt = SegModel_CogTAA(None,
                pretrain_img_size=384,
                patch_size=4,
                # Change this according to used Swin model
                embed_dim=96,
                depths = [2, 2, 6, 2],
                num_heads=[3, 6, 12, 24],
                window_size=7,
    ...
    def forward(self,x,z,y,toa,w):
        ...
        for i in range(x.size(1)):
            ...
            x = self.cross_vlt(x1, tokens_tensor, input_masks_tensors)
            ...
            current_loss=(5*L1+L2).mean()

            #compute gradients only when training
            if self.training:
                current_loss.backward()
                
            # print(f"current loss: {current_loss}")
            total_loss += current_loss.item()
            all_output.append(output1.detach().cpu())

        return total_loss,all_output
