...
class BertAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False, vision_dim=768):
        super().__init__()
        if is_cross_attention:
            self.self = BertSelfAttention(config, is_cross_attention=is_cross_attention, vision_dim=vision_dim)
        else:    
            self.self = BertSelfAttention(config, is_cross_attention=is_cross_attention)
...
class BertStage1(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.layer = nn.ModuleList([BertLayer(config) for _ in range(config.num_hidden_layers)])
        self.proj = nn.Linear(config.hidden_size, 256)
    ...
    def forward(
        ...
        cls_token = self.proj(hidden_states[:,0,:])
        ...
        return outputs[0], cls_token  # last-layer hidden state, (all hidden states), (all attentions)

class BertStage(nn.Module):
    def __init__(self, config):
        super(BertStage, self).__init__()
        self.config = config

        # language query fusion layer
        self.crossattention = BertAttention(config, is_cross_attention=True, vision_dim=config.vision_dim)
        self.intermediate = BertIntermediate(config)
        self.output = BertOutput(config)
        ...
        if encoder_attention_mask is not None:
            if type(encoder_attention_mask) == list:
                ...
                for mask in encoder_attention_mask:
                    ...
                    extended_mask = extended_mask.to(dtype=torch.float16)  # fp16 compatibility
                    extended_mask = (1.0 - extended_mask) * -1e4
                    encoder_extended_attention_mask.append(extended_mask)
            else:
...
class BertModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.encoder = BertStage1(config)
    
        self._init_weights()
        
    def _init_weights(self):
        bert_pre = torch.load("CrossVLT/pretrained/pytorch_model.bin")
        ...
        for k, v in bert_pre.items():
            ...
            if 'layer.6' in k:
                break
            if 'cls' not in k and 'pooler' not in k:
    ...
    Return:
        ...
        encoder_outputs, cls_token = self.encoder(
            embedding_output,
        ...
        return sequence_output , extended_attention_mask, head_mask, cls_token 
